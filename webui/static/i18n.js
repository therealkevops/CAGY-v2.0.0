...
const LOCALES = {
  en: {
    ...
    offline_network_detail: 'The Antigravity engine is unreachable from this browser right now.',
    offline_autorefresh: 'I will refresh this page automatically when the connection is restored.',
    ...
    mic_insecure_origin: 'Voice input needs a secure connection. Open Antigravity over HTTPS or from localhost to use the microphone.',
    ...
    mcp_restart_hint: 'MCP servers and tools can be configured via the MCP Hub or mcp.json.',
    ...
    approval_gateway_unsupported: 'Approvals require a newer gateway. Upgrade the connected Antigravity gateway to enable this.',
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" may not work with your configured provider (${p}). Send anyway, or run \`agy model\` in your terminal to switch.`,
    ...
    cmd_skills: 'List available Antigravity skills',
  ...
  untitled:'Untitled',
    no_personalities: 'No personalities found (add them to ~/.agy/personalities/)',
    ...
    session_title_regenerate_desc: 'Ask Antigravity to generate a fresh title from the conversation',
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    settings_section_preferences_meta: 'Defaults and UI behavior for Antigravity Web UI.',
    ...
    composer_control_terminal: 'Terminal',
    ...
    settings_plugins_title: 'Skills & Rules',
    plugins_enable_toggle: 'Enable',
    settings_plugins_meta: 'View installed Antigravity skills, rules, and lifecycle extensions.',
    settings_plugins_empty: 'No custom skills or rules currently active in .gemini/ or skills/.',
    ...
    settings_help_meta: 'Resources and support for Antigravity WebUI.',
    settings_help_docs_label: 'Documentation',
    settings_help_docs_desc: 'Guides, configuration reference, and Antigravity WebUI documentation.',
    ...
    settings_aux_advanced_timeout_desc: 'Request timeout for this auxiliary task. Blank uses AGY default.',
    ...
    settings_label_dashboard_mode: 'Official Antigravity Dashboard',
    settings_desc_dashboard_mode: 'Show a nav-rail link when the official Antigravity dashboard is reachable. Public reverse-proxy URLs are stored as browser-only links and are never server-probed.',
    ...
    settings_desc_gateway_status: 'Status of the messaging gateway (Telegram, Discord, Slack, etc.)',
    ...
    kanban_work_queue_hint: 'This is the agent work queue. Create or triage a task, assign it, move it to Ready, then let the dispatcher claim it.',
    ...
    kanban_assignee_profiles_label: 'Antigravity profiles',
    ...
    tab_dashboard: 'Antigravity Dashboard',
    ...
    insights_skill_usage_footer: 'Counts from skills catalog',
    ...
    settings_desc_voice_mode: 'Show the voice-mode button (audio waveform) next to the dictation mic. Lets you speak naturally — AGY auto-sends after a pause and reads replies aloud. Requires a browser that supports both speech recognition and TTS.',
    ...
    settings_desc_sync_insights: 'Mirrors WebUI token usage to state.db so agy /insights includes browser session data. Off by default.',
    ...
    password_env_var_locked: 'The WEBUI_PASSWORD or AGY_WEBUI_PASSWORD environment variable is currently set and takes precedence. Unset it and restart the server to manage the password from here.',
    password_env_var_locked_placeholder: 'Locked: WEBUI_PASSWORD env var is set',
    disable_auth: 'Disable Auth',
    sign_out: 'Sign Out',
    settings_label_shutdown: 'Stop the WebUI server',
    ...
    settings_shutdown_confirm_title: 'Stop WebUI',
    settings_shutdown_confirm_message: 'Stop the WebUI server?',
    ...
    providers_oauth_config_yaml_hint: 'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run agy auth.',
    providers_oauth_not_configured_hint: 'Not authenticated. Run agy auth in the terminal to configure this provider.',
    providers_save: 'Save',
    ...
    onboarding_title: 'Welcome to CAGY Web UI',
    onboarding_lead: 'A quick guided setup will verify CAGY, save a real provider configuration, choose a workspace and model, and optionally protect the app with a password.',
    onboarding_back: 'Back',
    onboarding_continue: 'Continue',
    onboarding_skip: 'Skip setup',
    onboarding_skipped: 'Setup skipped — using existing config.',
    onboarding_open: 'Open CAGY',
    onboarding_step_system_title: 'System check',
    onboarding_step_system_desc: 'Verify CAGY Agent and config visibility.',
    onboarding_step_setup_title: 'Provider setup',
    onboarding_step_setup_desc: 'Save the minimum CAGY provider config.',
    ...
    onboarding_notice_system_ready: 'CAGY Agent looks reachable from the Web UI.',
    onboarding_notice_system_unavailable: 'CAGY Agent is not fully available yet. Bootstrap can install it, but provider setup may still require a terminal.',
    onboarding_check_agent: 'CAGY Agent',
    ...
    onboarding_notice_setup_required: 'Choose a simple provider path here. Advanced OAuth flows still belong in the AGY CLI for now.',
    onboarding_notice_provider_choice_required: 'CAGY is installed, but you still need to choose a provider and save working credentials.',
    onboarding_notice_custom_base_url_required: 'CAGY has a saved provider/model selection, but the custom provider still needs a base URL. Add the API key too if that server requires one.',
    onboarding_notice_provider_auth_required: 'Provider "{0}" is configured but not yet authenticated. Run "agy auth" or "agy model" in a terminal to complete setup, then reload the Web UI.',
    onboarding_notice_provider_api_key_required: 'CAGY has a saved provider/model selection but still needs the API key required to chat.',
    onboarding_notice_setup_already_ready: 'A working CAGY provider setup is already detected. You can keep it or replace it here.',
    onboarding_oauth_provider_ready_title: 'Provider already authenticated',
    onboarding_oauth_provider_ready_body: 'This instance is configured to use an OAuth provider (<strong>{provider}</strong>) that was set up via the AGY CLI. No API key is needed here — click Continue to finish setup.',
    onboarding_oauth_provider_not_ready_title: 'OAuth provider not yet authenticated',
    onboarding_oauth_provider_not_ready_body: 'This instance is configured to use <strong>{provider}</strong>, which uses OAuth rather than an API key. Run <code>agy auth</code> or <code>agy model</code> in a terminal to authenticate, then reload the Web UI.',
    ...
    onboarding_api_key_help_prefix: 'Saved as a secret in your CAGY .env file using',
    ...
    onboarding_workspace_help: 'Pick the model CAGY should use for new chats after setup completes.',
    ...
    cron_script_path_hint: 'Resolved under ~/.agy/scripts/ unless an absolute path. Edit the script file on the server to change behavior.',
    ...
    composer_mobile_terminal: 'Terminal',
...
function setLocale(lang) {
  _locale = LOCALES.en;
  try { localStorage.setItem('agy-lang', 'en'); } catch (_) {}
  document.documentElement.lang = _locale._speech || 'en-US';
}
