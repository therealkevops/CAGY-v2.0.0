...
WORKSPACE_GIT_DESTRUCTIVE_ENV = "AGY_WEBUI_WORKSPACE_GIT_DESTRUCTIVE"
WORKSPACE_GIT_DESTRUCTIVE_ENV_LEGACY = "HERMES_WEBUI_WORKSPACE_GIT_DESTRUCTIVE"
_GIT_ENV_SCRUB_KEYS = (
    "GIT_DIR",
    "GIT_WORK_TREE",
    "GIT_CONFIG_GLOBAL",
...
_AGY_BRANCH_SWITCH_STASH_PREFIX = "hermes-webui branch switch"
...
def workspace_git_destructive_enabled() -> bool:
    return (os.getenv(WORKSPACE_GIT_DESTRUCTIVE_ENV) or os.getenv(WORKSPACE_GIT_DESTRUCTIVE_ENV_LEGACY, "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _stash_subject_parts(subject: str) -> tuple[str, str] | None:
    ...
    if not branch or not message.startswith(_AGY_BRANCH_SWITCH_STASH_PREFIX):
        return None
    return branch, message


def _agy_branch_switch_stashes(ctx: GitContext) -> list[dict]:
...
def _restore_branch_switch_stash_locked(ctx: GitContext, branch: str) -> dict:
    ...
    if _dirty_worktree(ctx):
        return {}
    for item in _agy_branch_switch_stashes(ctx):
...
def git_stash_and_checkout(
    ...
    stash_name = f"{_AGY_BRANCH_SWITCH_STASH_PREFIX} to {target_label}".strip()
    restored: dict = {}
    with _git_mutation_lock(ctx):
        _validate_checkout_request_locked(ctx, ref, mode, new_branch)
        if workspace_git_destructive_enabled() and _has_repo_local_filters(ctx.repo_root, _clean_git_env()):
