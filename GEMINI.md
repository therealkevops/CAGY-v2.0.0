...
- **Knowledge Note Placement**: When generating domain documentation, technical briefs, or research summaries for any workspace, always store them directly in `/workspace/knowledge/spaces/<space_id>/notes/` (and reference them in the project). Writing to the Knowledge Vault ensures notes are immediately indexed in the 2D force-directed graph, compiled into `.gemini/rules/knowledge_vault.md`, and retained across all turns.
